...
test('loads a BAM and changes the selected contig', async ({ page }) => {
  ...
  await expect(page.getByRole('checkbox', { name: 'Highlight differences' })).toBeDisabled()
  ...
  await expect(page.getByRole('checkbox', { name: 'Highlight differences' })).toBeEnabled()
  await page.getByRole('checkbox', { name: 'Highlight differences' }).check()
  await expect(page.getByLabel('Alignment viewport')).toHaveAttribute('data-colour-mode', 'differences')
